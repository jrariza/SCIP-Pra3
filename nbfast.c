...
float getElapsedTime(struct timespec startTime, struct timespec endTime) {
    float seconds = (endTime.tv_sec - startTime.tv_sec);
    float nseconds = (endTime.tv_nsec - startTime.tv_nsec);
    return (float) (seconds + nseconds / BILLION);
}
...
void printPartialStatistics(struct Partition *p) {
    ...
    printf("%sThread %i Stats Iter %i ## "
           ...
           "Thread Time: %.6f\tTotal Time: %.6f\tAverage Iter Time: %.6f\tUnbalance: %.3f%%\n%s",
           GREEN, p->id, count,
           p->numParts, p->first, p->last - 1, s.evalPart, s.deletedPart, s.simplPart,
           s.computTime, s.totalComputTime, s.totalComputTime / count, p->stats.loadImbalance * 100, RESET);
}
...
void threadRoutine(struct Partition *attr) {
    ...
    while (count <= steps) {
        ...
        if (count % M == 0) {
            ...
            float averageBalance = global.computTime / nThread;
            float ratio = (s->computTime - averageBalance) / averageBalance;
...
void loadBalancingGive(int i, double ratio, bool *done) {
    float unbalanceLeft = 0, unbalanceRight = 0;
    int left = i - 1;
    int right = i + 1;
    if (i != 0) {
        float leftDifference = attrs[i].stats.computTime - attrs[left].stats.computTime;
        if (leftDifference > 0)
            unbalanceLeft = leftDifference /
                            fabsf((attrs[i].stats.computTime + attrs[left].stats.computTime) / 2);
    }
    if (i != nThread - 1) {
        float rightDifference = attrs[i].stats.computTime - attrs[right].stats.computTime;
        if (rightDifference > 0)
            unbalanceRight = rightDifference /
                             fabsf((attrs[i].stats.computTime + attrs[right].stats.computTime) / 2);
    }
    if (unbalanceLeft == 0 && unbalanceRight == 0) return;

    int particlesToGive = ceil((ratio) * (attrs[i].numParts));
    int toGiveLeft = floor(particlesToGive * (unbalanceLeft / (unbalanceLeft + unbalanceRight)));
    int toGiveRight = floor(particlesToGive * (unbalanceRight / (unbalanceLeft + unbalanceRight)));
    ...
    *done = true;
    ...
    printf("%s""Thread %i LoadBalancing GIVE Iter %i ## "
           ...
           unbalanceLeft, toGiveLeft,
           unbalanceRight, toGiveRight
    );
...
void loadBalancingTake(int i, double ratio, bool *done) {
    ...
    int right = i + 1;
    if (i != 0) {
        float leftDifference = attrs[left].stats.computTime - attrs[i].stats.computTime;
        if (leftDifference > 0)
            unbalanceLeft = leftDifference /
                            fabsf((attrs[i].stats.computTime + attrs[left].stats.computTime) / 2);
    }
    if (i != nThread - 1) {
        float rightDifference = attrs[right].stats.computTime - attrs[i].stats.computTime;
        if (rightDifference > 0)
            unbalanceRight = rightDifference /
                             fabsf((attrs[i].stats.computTime + attrs[right].stats.computTime) / 2);
    }
    ...
    int toTakeLeft = floor(particlesToTake * (unbalanceLeft / (unbalanceLeft + unbalanceRight)));
    int toTakeRight = floor(particlesToTake * (unbalanceRight / (unbalanceLeft + unbalanceRight)));
    ...
    *done = true;
    ...
    printf("%sThread %i LoadBalancing TAKE Iter %i ## "
           ...
           CYAN, i, count,
           attrs[i].numParts, attrs[i].first, attrs[i].last - 1, -ratio * 100,
           unbalanceLeft, toTakeLeft,
           unbalanceRight, toTakeRight
    );
...
void applyBalancingPolicy() {
    ...
    for (int i = 0; i < nThread; i++) {
        float imbalanceRatio = attrs[i].stats.loadImbalance;
        if (!lastDidTake && imbalanceRatio > BALANCE_THRESHOLD && attrs[i].numParts > 1)
            loadBalancingGive(i, imbalanceRatio, &iDidGive);
        else if (!lastDidGive && imbalanceRatio < -BALANCE_THRESHOLD) loadBalancingTake(i, -imbalanceRatio, &iDidTake);
...
int main(int argc, char *argv[]) {
    ...
    if (graphicsEnabled) {
        ...
        while (!glfwWindowShouldClose(window) && count <= steps) {
            ...
            pthread_mutex_lock(&mutexStats);
            while (threadsFinished < nThread)pthread_cond_wait(&varCond, &mutexStats);
